...
#' @param saco Object with simulated community data created by \code{sample.comm.2} function.
...
draw.ecospace <- function (saco, plot.grad = c(1,2), resolution = 200, colors = NULL, species = NULL, asp = NULL, sample.pch = 3, sample.cex = .5, species.pch = 16, species.cex = 1, xlab = 'gradient 1', ylab = 'gradient 2', axes = TRUE, box = TRUE, ...)
{
  ...
  if (is.null (species)) species <- 1:saco$S
  
  graphics::plot.new ()
  graphics::plot.window (xlim = c(0, resolution), ylim = c(0, resolution), asp = asp)
  graphics::title (xlab = xlab, ylab = ylab) #, ...)  # later return back
  
  gr1 <- round (seq (1, saco$args.simcom$gr.length[saco$grad[1]], length = resolution))
  gr2 <- round (seq (1, saco$args.simcom$gr.length[saco$grad[2]], length = resolution))
  ...
  graphics::plot.window (xlim = c(0, saco$args.simcom$gr.length[saco$grad[1]]), ylim = c(0, saco$args.simcom$gr.length[saco$grad[2]]), asp = asp)
